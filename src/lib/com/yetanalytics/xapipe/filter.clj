...
(comment
  ...
  (let [profile-url "dev-resources/profiles/calibration_strict_pattern.jsonld"
        fsm-map (per/compile-profiles->fsms [(get-profile profile-url)]
                                            )
        [a b c :as ss] (sup/gen-statements
                        3
                        ...
                        :personae [{:name "Test Subjects",
                                    ...
                                    [{:name "alice",
                                      ...
                                      :objectType "Agent"}]}])]
    ;; Trying to match the second statement first will pass validation
    ;; BUT be properly rejected

    (as-> {} m
        (per/match-statement fsm-map m a)
        #_(per/match-statement fsm-map m b)
        #_(per/match-statement fsm-map m c)))
        ...
        [a b c :as ss] (sup/gen-statements
                        ...
                        :personae [{:name "Test Subjects",
                                    ...
                                    [{:name "alice",
                                      :mbox "mailto:alice@example.org",
                                      :objectType "Agent"}]}])
        [d e f :as other-ss] (sup/gen-statements
                              3
                              :profiles ["dev-resources/profiles/calibration_a.jsonld"]
                              :personae [{:name "Test Subjects",
                                          :objectType "Group",
                                          :member
                                          [{:name "bob",
